...
{    
    ...
    public function __construct() {
        parent::__construct();
        $this->mydb = $this->load->database('default', true);
    }
    ...
    {
        if($this->mydb->set($data)->insert($this->tableName())) {
    ...
    {
        return $this->mydb->where($where)->update($this->tableName(), $attributes);
    }
    ...
    {
        return $this->mydb->delete($this->tableName(), $where);
    }
    ...
    {
        $where[$this->primaryKey()] = $pk;
        $query = $this->mydb->from($this->tableName())->where($where)->get();
    ...
    {
        $this->mydb->from($this->tableName())->where($where);
        if($sort !== NULL) {
    ...
    {
        return $this->mydb->from($this->tableName())->where($where)->count_all_results();
    }
    ...
    {
        $query = $this->mydb->query($sql, $param);
        return $query->result_array();
    }
}
