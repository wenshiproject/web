<!--右侧-->
	<div class="sdk_right">
		<div class="sdk_rightnav">当前位置：许愿清单 > 编辑信息</div>
		<div class="sdk_righcon">
			...
			<table cellspacing="0" cellpadding="0" border="0" width="100%" class="sdk_table1">
              	<tr>
              		<td align="right" width="150"><lable class="colred">*</lable>信息标题：</td><td><input type="text" class="sdk_zhucetext" size="15" name="title" value="<?php echo $rs['title']?>" class="{required:true,messages:{required:'不能为空'}}"/>
              		</td>
              </tr>
               <tr>
              		<td align="right" width="150"><lable class="colred">*</lable>信息全文：</td><td><input type="text" class="sdk_zhucetext" size="15" name="content" value="<?php echo $rs['content']?>" class="{required:true,messages:{required:'不能为空'}}" />
              		</td>
              </tr>
			   <tr>
...
function delimg(key, id, index){
	$.ajax({
	    url:'/admin/wish/delimg',// 跳转到 action  
	    data:{
	        key : key,
	        id	: id,
	    },
