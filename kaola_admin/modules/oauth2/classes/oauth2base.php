...
//define("OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME", 604800);// 7天
define("OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME", 2419200);// 28天
...
define("OAUTH2_TOKEN_PARAM_NAME_SAVE_IN_SDK", "autoStr");
...
abstract class Oauth2base {
  ...
  /**
   * save access token here after auth.
   * @var string
   */
  ...
  private $signStr = null;
  private $autoStr = null;
  private $suid    = null;
  
  /**
	 * @return the $autoStr
	 */
	public function get_autoStr() {
		return $this->autoStr;
	}

/**
	 * @param field_type $autoStr
	 */
	public function set_autoStr($autoStr) {
	 ...
	 * @return the $signStr
	 */
	public function get_signStr() {
		return $this->signStr;
	}

/**
	 * @param field_type $signStr
	 */
	public function set_signStr($signStr) {
...
public function get_access_token()
  ...
  abstract protected function getServerSecret($client_id);
   ...
   */
  abstract protected function setAccessToken($oauth_token, $client_id, $expires, $scope = NULL,$username,$suid);
  ...
  public function verifyAccessToken($scope = NULL, $exit_not_present = TRUE, $exit_invalid = TRUE, $exit_expired = TRUE, $exit_scope = TRUE, $realm = NULL) {
    ...
    $key = $this->getServerSecret($token["client_id"]);
    $timestamp = time();
    $this->set_expires_in($expires_in);
    $this->set_username($token["username"]);
    $this->set_suid($token['suid']);
    $this->set_autoStr($this->encodeAccessTokenToSDK($token_param));
    $this->set_timestamp($timestamp);
    $this->set_signStr($this->genSinStr($token["username"], $timestamp, $key));
    if ($token === NULL)
  ...
  private function genSinStr($username,$timestamp,$key)
  {
  ...
  private function getAccessTokenParams() {
    ...
    if (isset($_GET[OAUTH2_TOKEN_PARAM_NAME])) {
      ...
      $this->access_token = $_GET[OAUTH2_TOKEN_PARAM_NAME];
      return $_GET[OAUTH2_TOKEN_PARAM_NAME];
    }

    if (isset($_GET[OAUTH2_TOKEN_PARAM_NAME_SAVE_IN_SDK])) {
      $this->access_token = $this->decodeAccessTokenToSDK($_GET[OAUTH2_TOKEN_PARAM_NAME_SAVE_IN_SDK]);
      return $this->access_token;
    }

    if (isset($_POST[OAUTH2_TOKEN_PARAM_NAME]))
    {
	  $this->access_token = $_POST[OAUTH2_TOKEN_PARAM_NAME];
  ...
  public function finishClientAuthorization($is_authorized, $params = array()) {
    ...
    extract($params);
    $user = Auth_Api::instance()->get_user();
    ...
    if ($is_authorized === FALSE) {
      ...
      {
        $result["fragment"] = $this->createAccessToken($client_id, $scope,$user['user_name'],$user['suid']);
      }
    }
    
    // 针对风云天下的需求，增加额外参数
    $accessToken = $this->get_access_token();
//    Debug::file_put_contents($user['user_name']);
//    Debug::file_put_contents(Session::instance(null,$accessToken)->as_array());
    $result["query"]["username"] =  !empty($result["query"]["username"]) ? $result["query"]["username"] : $user['user_name'];//针对自动登录处理
    $result["query"]["timestamp"] =  time();
    $key = $this->getServerSecret($client_id);
    $result["query"]["signStr"] =  $this->genSinStr($result["query"]["username"], $result["query"]["timestamp"], $key);
    $result["query"]["autoStr"] =  $this->encodeAccessTokenToSDK($accessToken);
   ...
   * 保存到SDK客户端的token加密
   */
  public function encodeAccessTokenToSDK($accessToken)
  {
  	$addStr = substr($accessToken, 10,1);
  	return $accessToken.$addStr;
  }
   ...
   * 保存到SDK客户端的token解密
   */
  private function decodeAccessTokenToSDK($encodedAccessToken)
  {
  	return substr($encodedAccessToken, 0,strlen($encodedAccessToken)-1); 
  }
   ...
   * @ingroup oauth2_section_3
   */
  public function doRedirectUriCallback($redirect_uri, $params) {
   ...
   * @ingroup oauth2_section_4
   */
  public function createAccessToken($client_id, $scope = NULL,$username,$suid) {
    $token = array(
    ...
    $this->setAccessToken($token["access_token"], $client_id, time() + $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME), $scope, $username,$suid);
  ...
  public  function genAccessToken() {
  	...
  	{
	  	$this->access_token = md5(base64_encode(pack('N6', mt_rand(), mt_rand(), mt_rand(), mt_rand(), mt_rand(), uniqid())));
  	}
  	// 把access token 作为sessionid
  	if (session_id()){
	  	session_id($this->access_token);
  	}
    return $this->access_token;
  }
