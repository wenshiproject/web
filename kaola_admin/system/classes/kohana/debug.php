...
class Kohana_Debug {
	...
	/**
	 * print_r 函数强化，可以打印出调用的类和函数及在哪个文件哪一行，
	 * 方便调试及后面删除代码
	 * 
	 * @param $result
	 */
	...
	{
	    print_r(self::formatOutPut($result,debug_backtrace()));
	}
	public static function p($result)
	{
	    print_r(self::formatOutPut($result,debug_backtrace()));
	}
	 ...
	 * @param string $result
	 */
	public static function ph($result)
	{
		echo '<pre>';
	    print_r(self::formatOutPut($result,debug_backtrace()));
		echo '</pre>';
	...
	public static function file_put_contents($result)
	{
		if (!is_dir('/data/logs'))
		{
			mkdir('/data/logs',0777,true);
		}
		file_put_contents('/data/logs/DebugHelper.log',self::formatOutPut($result,debug_backtrace()),FILE_APPEND);
	}
	
	private static function formatOutPut($result,$trace,$newLine="\n")
	{
		$text = '-_-_-['.date("Y-m-d H:i:s").'] ';
		if (isset($trace[1]))
		{
			// 如果是类里面调用，才打印类信息：
